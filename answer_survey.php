...
$blok = $conn->query("SELECT id from answers where survey_id = $id and user_id ={$_SESSION['login_id']}")->num_rows;
if($blok >= 1){
	echo "<script>alert('Maaf, Anda sudah mengisi survey ini!'); window.location.href = 'survey.php?page=survey_widget';</script>";
	exit();
}
?>
<div class="col-lg-12">
	<div class="row">
			...
			<div class="card card-outline card-primary">
				...
				<div class="card-body p-0 py-2">
					<div class="container-fluid">
						...
						<p>Berakahir: <b><?php echo date("M d, Y",strtotime($end_date)) ?></b></p>
					</div>
		...
		<div id="msg"></div>
			<div class="card card-outline card-success">
				<div class="card-header">
					<h3 class="card-title"><b>Survey</b></h3>
				</div>
				...
				<div class="card-footer border-top border-success">
					<div class="d-flex w-100 justify-content-center">
						<button class="btn btn-sm btn-flat bg-gradient-primary mx-1" form="manage-survey">Simpan Jawaban</button>
						<button class="btn btn-sm btn-flat bg-gradient-secondary mx-1" type="button" onclick="location.href = 'survey.php?page=survey_widget'">Cancel</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$('#manage-survey').submit(function(e){
		...
		// Validasi untuk pertanyaan tipe textfield
		var textfieldAnswers = $('textarea[name^="answer"]');
		var isTextfieldEmpty = textfieldAnswers.filter(function() {
			return this.value === '';
		}).length > 0;
		
		// Validasi untuk pertanyaan tipe checkbox

		var checkboxAnswers = $('input[type="checkbox"][name^="answer"]');
		var isCheckboxAnswered = checkboxAnswers.filter(function() {
    	return this.checked;
		}).length > 0;

		var isCheckboxUnchecked = checkboxAnswers.filter(function() {
    	return !this.checked;
		}).length > 0 && !isCheckboxAnswered;

		
		// Jika ada pertanyaan tipe textfield atau checkbox yang belum dijawab, tampilkan pesan
		if (isTextfieldEmpty || isCheckboxUnchecked) {
		...
		$.ajax({
			...
			success: function(resp) {
				if(resp == 1){
					alert_toast("Terimakasih sudah menjawab.",'success')
					setTimeout(function(){
